...
def compute_a(z):
    ...
    for element in z:
        if element > 100:
            ...
            e_z.append(np.exp(element))
    e_z = np.array(e_z)
    a = np.mat(e_z/np.sum(e_z)).T


    #norm = np.exp(z - np.max(z))
    #a = norm/norm.sum()
    #########################################
    return a
...
def compute_dz_dW(x,c):
    ...
    dz_dW = np.zeros((1, p))
    for num in range(c):
        dz_dW = np.append(dz_dW, mat, axis=0)
        num += 1
    dz_dW = dz_dW[1:]
...
def compute_dz_db(c):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz_db = np.matrix(np.zeros((c, 1)) + 1.0)
...
def backward(x,y,a):
    ...
    dz_dW = compute_dz_dW(x, c=a.shape[0])
    dz_db = compute_dz_db(c=a.shape[0])
...
def compute_dL_dz(dL_da,da_dz):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_dz = np.sum(np.multiply(dL_da.T, da_dz.T), axis=1)
